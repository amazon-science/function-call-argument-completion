...
from dataset import finetuneDataset, EvalDataset, lineDataset, CallDataset

from transformers import (WEIGHTS_NAME, AdamW, get_linear_schedule_with_warmup,
                          BertConfig, BertForMaskedLM, BertTokenizer,
                          GPT2Config, GPT2LMHeadModel, GPT2Tokenizer,
                          OpenAIGPTConfig, OpenAIGPTLMHeadModel, OpenAIGPTTokenizer,
                          RobertaConfig, RobertaForMaskedLM, RobertaTokenizer,RobertaModel,
...
def load_and_cache_examples(args, tokenizer, evaluate=False):
    if args.do_train_fc:
        dataset = CallDataset(tokenizer, args, logger, file_type='dev' if evaluate else 'train', 
                                block_size=None)
    elif args.not_pretrain:
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    if args.model_type in ["rnn", "unixcoder"]:
...
def eval_acc(args, model, tokenizer, file_type='test'):
    ...
    for step, batch in enumerate(eval_dataloader):
        ...
        for x, y in zip(all_pred, all_gt):
            if y not in ["<s>", "</s>", "<EOL>", "<pad>", "<decoder-only>"]:
                total += 1
                if x == y:
                    correct += 1
...
def eval_call_completion(args, model, tokenizer, file_type='test'):
    """
        ...
        for idx in idxs:
            ...
            if tokenizer.convert_ids_to_tokens(idx)[0] == '\u0120':
                ...
                if not codes.endswith(" "):
                    ...
                    codes += to_add
            elif idx in [tokenizer.bos_token_id, tokenizer.eos_token_id, tokenizer.sep_token_id, tokenizer.pad_token_id, tokenizer.unk_token_id]:
                continue
            elif to_add.startswith("<NUM_LIT") or to_add.startswith("<STR_LIT"):
                to_add = to_add[8:]
                to_add = to_add.rstrip(">")
                codes += to_add
            elif to_add.startswith("<CHAR_LIT"):
                to_add = to_add[9:]
                to_add = to_add.rstrip(">")
                codes += to_add
            else:
            ...
            if to_add == "(":
                right+=1
        #codes = ".".join([x.strip() for x in codes.split(".")])
        #codes = "=".join([x.strip() for x in codes.split("=")])
        return codes.strip(" ")

    dataset = CallDataset(tokenizer, args, logger, file_type=file_type, block_size=args.block_size-100)
    ...
    if args.langs == "python":
        break_ids = [tokenizer.eos_token_id]
    else:
        ...
        with torch.no_grad():
            if args.model_type == "unixcoder":
                ...
                for i in range(inputs.shape[0]):
                    ...
                    beam = Beam(beam_size, inputs[i][-1].cpu().data, break_ids, tokenizer)
                    input_ids = None
                    for _ in range(100): 
                        if beam.done():
                            break
            ...
            for pred in p:
                ...
                for x in ["=", "+", "-", ".", "*", "/", "[", "]", ")", "("]:
                    text = x.join([y.strip() for y in text.split(x)])

                text = check(text)
                gold = check(gt[0].strip())

                preds.append(text)
                gts.append(gold)
                edit_sim += fuzz.ratio(text, gold)
                em += 1 if text == gold else 0
        if (step+1) % args.logging_steps == 0:
            logger.info(f"{step+1} are done!")

    saved_file = os.path.join(args.output_dir, "predictions_call.txt")
...
def main():
    ...
    parser.add_argument("--do_eval_fc", default="", type=str, help="Whether to run function call completion on the test set.")
    parser.add_argument("--block_size", default=1024, type=int,
                        help="Optional input sequence length after tokenization."
                             "The training dataset will be truncated in block of this size for training."
                             "Default to the model max input length for single sentence inputs (take into account special tokens).")
    parser.add_argument("--new_process", action='store_true',
                        help="Whether to new_process.")
    parser.add_argument("--do_train", action='store_true',
                        help="Whether to run training.")
    parser.add_argument("--do_train_fc", action='store_true',
                        help="Whether to run function call training.")
    parser.add_argument("--do_eval", action='store_true',
                        help="Whether to run eval on the dev set.")
    parser.add_argument("--use_implementation", action='store_true',
                        help="Whether to use implementation on the dev set.")
    parser.add_argument("--usages", default=0, type=int)
    parser.add_argument("--evaluate_during_training", action='store_true',
                        help="Run evaluation during training at each logging step.")
    parser.add_argument("--not_process", action='store_true')
    ...
    if pretrained:
        if args.model_type == "rnn":
            tokenizer = tokenizer_class.from_pretrained(pretrained, do_lower_case=args.do_lower_case, sep_token='<EOL>', bos_token='<s>', eos_token='</s>', pad_token='<pad>', unk_token='<unk>', additional_special_tokens=special_tokens)
            ...
            tokenizer = tokenizer_class.from_pretrained("microsoft/unixcoder-base", do_lower_case=args.do_lower_case, sep_token='<EOL>', bos_token='<s>', eos_token='</s>', pad_token='<pad>', unk_token='<unk>', additional_special_tokens=special_tokens)
            ...
            from beam import Decoder
            model = Decoder(decoder=encoder,tokenizer=tokenizer, config=config,
                        beam_size=5,max_length=100,
        ...
        tokenizer = tokenizer_class.from_pretrained(args.tokenizer_dir, sep_token='<EOL>', bos_token='<s>', eos_token='</s>', pad_token='<pad>', unk_token='<unk>', additional_special_tokens=special_tokens)
    ...
    if args.do_eval:
        ...
        if len(args.do_eval_fc)>0:
            eval_call_completion(args, model, tokenizer, 'test')
        else:
