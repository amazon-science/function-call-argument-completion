...
from transformers import PLBartForConditionalGeneration, PLBartTokenizer, AutoConfig, AutoModelForCausalLM, AutoTokenizer
import logging
...
MODEL_CLASSES = {'roberta': (RobertaConfig, RobertaModel, RobertaTokenizer),
                 ...
                 'gpt2': (GPT2Config, GPT2LMHeadModel, AutoTokenizer),
                 }
...
def build_or_load_gen_model(args):
    ...
    if args.model_type == 'roberta' and args.model_name_or_path == "microsoft/unixcoder-base":
        if args.decoder_only:
            ...
            model = Decoder(decoder=encoder,config=config,
    ...
    elif args.model_type in ["codegen", "gpt2"]:
        tokenizer = tokenizer_class.from_pretrained(args.tokenizer_name, sep_token='</s>', cls_token='<s>', eos_token='</s>', pad_token='<pad>', unk_token='<unk>')
        model = model_class.from_pretrained(args.model_name_or_path)
        logger.info("Resize embedding")
...
class Decoder(nn.Module):
    ...
    def forward(self, source_ids, labels=None):     
